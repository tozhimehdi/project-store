<!DOCTYPE html>
<html lang="fa" class="light-style customizer-hide" dir="rtl" data-theme="theme-default" data-assets-path="{{ asset('/admin') }}/" data-template="vertical-menu-template">
  <head>
    <meta charset="utf-8">
  ...
  <body>
      ...
      <div class="authentication-inner py-4">
        ...
        <div class="card">
          <div class="card-body">
            ...
            <form id="twoStepsForm" action="{{ route('verify.otp') }}" method="POST">
                @CSRF
              <div class="mb-3">
              ...
              <div id="otpTimer" class="alert alert-info" style="display: none">
                    <strong>زمان باقی مانده: </strong> <span id="timer"></span>
              </div>
            
              <div class="text-center" id="resendOtp" style="display: none">
                کد را دریافت نکردید؟
                <a href="{{ route('resend.otp') }}"> ارسال دوباره </a>
              </div>
    ...
    <script src="{{ asset('admin/js/pages-auth-two-steps.js') }}"></script>
        ...
        let expiresAt = "{{ session('otp_expires_at') }}"; // زمان انقضای OTP از Session
        let now = new Date();
        let timeLeft = Math.floor((new Date(expiresAt) - now) / 1000); // زمان باقی‌مانده به ثانیه
        ...
        if (timeLeft > 0) {
            ...
            const countdown = setInterval(function() {
                ...
                if (timeLeft < 0) {
                    ...
                    // می‌توانید اینجا کدی برای غیرفعال کردن دکمه تأیید OTP اضافه کنید
                }
